...
function getStats() {
  try {
    ...
    return JSON.parse(fs.readFileSync(statsPath) as any);
  } catch (e) {
...
function loadSession(jobId: string | null = null) {
  const sPath = getSessionPath(jobId || '');
  if (fs.existsSync(sPath)) {
    try {
      return JSON.parse(fs.readFileSync(sPath) as any);
    } catch (e) { return null; }
  }
  if (!jobId && fs.existsSync(sessionFile)) {
     try { return JSON.parse(fs.readFileSync(sessionFile) as any); } catch(e) { return null; }
  }
  return null;
}
...
app.get('/api/stream/:jobId', (req, res) => {
  ...
  if (!job) {
    ...
    if (fs.existsSync(jobFile)) {
      try {
        const session = JSON.parse(fs.readFileSync(jobFile) as any);
...
app.get('/api/download', (req, res) => {
  const { path: filePath, jobId } = req.query as any;
